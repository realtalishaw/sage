...
const ChatBubble = ({ message }: { message: Message }) => {
  ...
  return (
    <div className={`flex flex-col mb-1.5 ${isMe ? 'items-end' : 'items-start'} relative`}>
      ...
      {isMe && message.status && (
        <span className="text-[11px] text-gray-500 mt-0.5 mr-1 font-medium">
...
export default function IPhoneMockup({ messages, contactName = 'sage', contactEmoji = '🌱' }: IPhoneMockupProps) {
  return (
    <PhoneFrame>
      ...
      <div className="px-4 pt-3 pb-10 flex items-center gap-2 bg-black relative z-40">
        ...
        <div className="flex-1 relative">
          <input
            ...
            className="w-full bg-[#1c1c1e] border border-white/10 rounded-full py-1.5 pl-4 pr-10 text-[16px] text-white focus:outline-none focus:border-blue-500/50 transition-all placeholder:text-gray-600"
            readOnly
          />
          <button
